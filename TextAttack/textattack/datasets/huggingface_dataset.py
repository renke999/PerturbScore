...
class HuggingFaceDataset(Dataset):
    ...
    def __init__(
        ...
        # NOTE 20221211 code for sst2
        if name_or_dataset == 'sst2':
            self._dataset = datasets.load_dataset("glue", "sst2")[split]
        elif isinstance(name_or_dataset, datasets.Dataset):
        ...
        # NOTE ========================== code for probe ===========================
        # imdb
        if name_or_dataset == 'imdb':
            # NOTE 20221120 imdb有25000个，测试100个，防止保存的文件太大
            # self._dataset = self._dataset.filter(lambda example, idx: idx % 250 == 0, with_indices=True)
            # print("dataset_name: {}, dataset_len: {} \n".format(name_or_dataset, len(self._dataset)))

            # NOTE 20221211 imdb有25000个，测试1000个
            self._dataset = self._dataset.filter(lambda example, idx: idx % 25 == 0, with_indices=True)
            print()
            print("**********" * 5)
            print("dataset_name: {}, dataset_len: {}".format(name_or_dataset, len(self._dataset)))
            print("**********" * 5)
            print()
        elif name_or_dataset == 'ag_news':
            # TODO 20221226 ag_news有120000个，1000个sample
            self._dataset = self._dataset.filter(lambda example, idx: idx % 120 == 0, with_indices=True)
            print()
            print("**********" * 5)
            print("dataset_name: {}, dataset_len: {}".format(name_or_dataset, len(self._dataset)))
            print("**********" * 5)
        ...
        # sst2
        elif name_or_dataset == 'sst2':
            # NOTE 20221211 imdb有67349个，测试约1000个
            self._dataset = self._dataset.filter(lambda example, idx: idx % 67 == 0, with_indices=True)
            print()
            print("**********" * 5)
            print("dataset_name: {}, dataset_len: {}".format(name_or_dataset, len(self._dataset)))
            print("**********" * 5)
